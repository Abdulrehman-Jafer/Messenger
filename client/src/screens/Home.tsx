...
import { findInContact, getLastItem } from "../utils/misc"
...
export default function Home() {
    ...
    useEffect(() => {
        if (chatSuccess && !isLoading) {
            ...
            console.log(chatData.result.chatspaces)
        }
    }, [chatLoading])
        ...
        <>
            <main className="bg-pink-red">
                <div className="bg-white bottom-boder-radius">
                    ...
                    <article className="flex flex-col last-child-border-bottom-white h-screen">
                        {(chats && chats.length) > 0 ? chats.map(c => {
                            // return <p>Hello</p>
                            return <Recent_Chat active_status={"2:34pm"} last_message={getLastItem(c.messages) || "No recent Message"} name={c.receiver.isSaved ? c.contact.saved_as : c.receiver.user._id} user_image={c.receiver.user.image} key={c._id} />
                        }) :
