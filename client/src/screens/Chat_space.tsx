...
const chat_space = {
    _id: "abcdefgh",
    between: [{ name: "Francine Ralley", image: dummy_image }, { name: "Sara", image: dummy_image }],
    messages: [
        ...
        // status: string
        {
            sender: { name: "Francine Ralley", image: dummy_image },
        ...
        {
            sender: { name: "Sara", image: dummy_image },
            message: "Hello! Saraa",
            create_at: "14:02",
            sent_to: { name: "Francine Ralley" },
            status: "sent"
        },
        {
            sender: { name: "Sara", image: dummy_image },
        ...
        {
            sender: { name: "Francine Ralley", image: dummy_image },
            message: "Now a days I'm focusing on traveliing. In the last summer, I visited Bangladesh",
            create_at: "14:02",
            sent_to: { name: "Sara" },
            staus: "sent"
        },
        ...
        {
            sender: { name: "Sara", image: dummy_image },
            message: "I have also interested on the nice green country, One of my cousin visited there. And I am really excited",
            create_at: "14:02",
            sent_to: { name: "Francine Ralley" },
            staus: "sent"
        },
...
export default function Chat_space() {
    ...
    const Connected_to = chat_space.between[0].name == Logged_In_User.name ? chat_space.between[0] : chat_space.between[1]
    return (
        <main className="flex flex-col">
            <section className="flex p-[1rem] justify-between items-center sticky top-0 z-10 backdrop-blur-3xl">
                ...
                <h2 className="text-[1.3rem]">{Connected_to.name}</h2>
                <div className="flex-shrink-0">
                    <img src={Connected_to.image} alt="contact_image" className="h-10 w-10 rounded-full" />
                </div>
            </section>
            <section className="flex flex-col p-[1rem] last-child-border-bottom-white">
                {chat_space.messages.map((m, i) => {
                    const stylingData = {
                        nextMsgSender: chat_space.messages[i + 1]?.sender,
                        nextMsgTime: chat_space.messages[i + 1]?.create_at
                    }
                    return <Message created_at={m.create_at} message={m.message} sender={m.sender} sent_to={m.sent_to} status={m.staus} nextMsgSender={stylingData.nextMsgSender} nextMsgTime={stylingData.nextMsgTime} key={crypto.randomUUID()} />
                })}
            </section>
            <section className="sticky bottom-0">
                <input type="text" className="border border-gray-400 outline-none w-full p-[1rem] bg-light-gray" placeholder="Type your message" />
            </section>
        </main>
    )
}
