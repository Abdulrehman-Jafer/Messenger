import { IoIosArrowBack } from "react-icons/io"
import Message from "../components/Message"
import { useNavigate } from "react-router-dom"
// import { useEffect, useRef } from "react"

const dummy_image = "data:image/jpeg;base64,"
const chat_space = {
    _id: "abcdefgh",
    user1_id: { name: "Francine Ralley", image: dummy_image },
    user2_id: { name: "Sara", image: dummy_image },
    chat: [
        // sender: { name: string, image: string },
        // message: string,
        // created_at: string,
        // sent_to: { name: string },
        // status: string
        {
            sender: { name: "Francine Ralley", image: dummy_image },
            message: "Hello? What's up?",
            create_at: "14:02",
            sent_to: { name: "Sara" },
            status: "sent"
        },
        {
            sender: { name: "Sara", image: dummy_image },
            message: "Hello! Saraa",
            create_at: "14:02",
            sent_to: { name: "Francine Ralley" },
            status: "sent"
        },
        {
            sender: { name: "Sara", image: dummy_image },
            message: "I am doing great! How are you today?",
            create_at: "14:02",
            sent_to: { name: "Francine Ralley" },
            status: "sent"
        },
        {
            sender: { name: "Francine Ralley", image: dummy_image },
            message: "Hmm, everything is fine",
            create_at: "14:02",
            sent_to: { name: "Sara" },
            staus: "sent"
        },
        {
            sender: { name: "Francine Ralley", image: dummy_image },
            message: "Now a days I'm focusing on traveliing. In the last summer, I visited Bangladesh",
            create_at: "14:02",
            sent_to: { name: "Sara" },
            staus: "sent"
        },
        {
            sender: { name: "Sara", image: dummy_image },
            message: "WOW! Amazing country",
            create_at: "14:02",
            sent_to: { name: "Francine Ralley" },
            staus: "sent"
        },
        {
            sender: { name: "Sara", image: dummy_image },
            message: "I have also interested on the nice green country, One of my cousin visited there. And I am really excited",
            create_at: "14:02",
            sent_to: { name: "Francine Ralley" },
            staus: "sent"
        },
        {
            sender: { name: "Francine Ralley", image: dummy_image },
            message: "Great",
            create_at: "14:02",
            sent_to: { name: "Sara" },
            staus: "sent"
        },
        {
            sender: { name: "Francine Ralley", image: dummy_image },
            message: "Hello? What's up?",
            create_at: "14:02",
            sent_to: { name: "Sara" },
            status: "sent"
        },
        {
            sender: { name: "Sara", image: dummy_image },
            message: "Hello! Saraa",
            create_at: "14:02",
            sent_to: { name: "Francine Ralley" },
            status: "sent"
        },
        {
            sender: { name: "Sara", image: dummy_image },
            message: "I am doing great! How are you today?",
            create_at: "14:02",
            sent_to: { name: "Francine Ralley" },
            status: "sent"
        },
        {
            sender: { name: "Francine Ralley", image: dummy_image },
            message: "Hmm, everything is fine",
            create_at: "14:02",
            sent_to: { name: "Sara" },
            staus: "sent"
        },
        {
            sender: { name: "Francine Ralley", image: dummy_image },
            message: "Now a days I'm focusing on traveliing. In the last summer, I visited Bangladesh",
            create_at: "14:02",
            sent_to: { name: "Sara" },
            staus: "sent"
        },
        {
            sender: { name: "Sara", image: dummy_image },
            message: "WOW! Amazing country",
            create_at: "14:02",
            sent_to: { name: "Francine Ralley" },
            staus: "sent"
        },
        {
            sender: { name: "Sara", image: dummy_image },
            message: "I have also interested on the nice green country, One of my cousin visited there. And I am really excited",
            create_at: "14:02",
            sent_to: { name: "Francine Ralley" },
            staus: "sent"
        },
        {
            sender: { name: "Francine Ralley", image: dummy_image },
            message: "Great",
            create_at: "14:02",
            sent_to: { name: "Sara" },
            staus: "sent"
        }
    ]
}



export default function Chat_space() {
    const navigate = useNavigate()
    const Logged_In_User = { name: "Sara", image: dummy_image }
    const Connected_to = chat_space.user1_id.name == Logged_In_User.name ? chat_space.user2_id : chat_space.user1_id
    return (
        <main className="flex flex-col">
            <section className="flex p-[1rem] justify-between items-center sticky top-0 z-10 backdrop-blur-3xl">
                <i className="text-[1.3rem] cursor-pointer" onClick={() => navigate("/chats")}><IoIosArrowBack /></i>
                <h2 className="text-[1.3rem]">{Connected_to.name}</h2>
                <div className="flex-shrink-0">
                    <img src={Connected_to.image} alt="contact_image" className="h-10 w-10 rounded-full" />
                </div>
            </section>
            <section className="flex flex-col p-[1rem] last-child-border-bottom-white">
                {chat_space.chat.map((m, i) => {
                    const stylingData = {
                        nextMsgSender: chat_space.chat[i + 1]?.sender,
                        nextMsgTime: chat_space.chat[i + 1]?.create_at
                    }
                    return <Message created_at={m.create_at} message={m.message} sender={m.sender} sent_to={m.sent_to} status={m.staus} nextMsgSender={stylingData.nextMsgSender} nextMsgTime={stylingData.nextMsgTime} key={crypto.randomUUID()} />
                })}
            </section>
            <section className="sticky bottom-0">
                <input type="text" className="border border-gray-400 outline-none w-full p-[1rem] bg-light-gray" placeholder="Type your message" />
            </section>
        </main>
    )
}
