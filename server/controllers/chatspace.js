...
export const getUserChatSpace = async (req, res, next) => {
  ...
  try {
    ...
    const modifiedChatSpaces = chatspaces.map((c) => {
      ...
      const isBlockedByReceiver = sender.blocked_by.includes(
        receiver.connected_to.public_number
      );
...
export const createChatSpace = async (req, res, next) => {
  ...
  try {
    ...
    const isBlocked = sender.blocked_by.includes(
      receiver.connected_to.public_number
    );
    ...
    if (!isBlocked) {
      if (doesExist) {
        const messageFrom = io_receiver.isSaved
          ? io_receiver.contact.saved_as
          : io_receiver.connected_to.public_number;
        io.to(receiver.connected_to.socketId).emit("receive-message", {
          message: modifiedMessage,
          messageFrom,
        });
      } else {
        io.to(receiver.connected_to.socketId).emit("new-messager", {
          chatspace: ioChatSpace,
          message: modifiedMessage,
        });
      }
    }
...
export const deleteAllMessageOfAChatSpace = async (req, res, next) => {
  ...
  try {
    const updates = await Message.updateMany(
      { belongsTo: chatspace_id },
      { $push: { deletedFor: user_id } }
    );
    await Chatspace.updateOne(
      { _id: chatspace_id },
      { $pull: { archived_for: user_id } }
    );
    return res.status(200).json({
      responseCode: 200,
...
export const addToArchive = async (req, res, next) => {
  const { chatspace_id, user_id } = req.body;
  try {
    const updatedChatspace = await Chatspace.findOneAndUpdate(
      { _id: chatspace_id },
      { $push: { archived_for: user_id } },
      { new: true }
    );
    if (!updatedChatspace) {
    ...
    return returnResponse(res, 200, "successfully added to archive", {
      updatedChatspace,
    });
  } catch (error) {
...
export const removeFromArchive = async (req, res, next) => {
  ...
  try {
    ...
    return returnResponse(res, 200, "removed from archive", {
      updatedChatspace,
    });
  } catch (error) {
