...
export const sendMessage = async (req, res, next) => {
  ...
  try {
    ...
    if (data.receiver.socketId) {
      io.to(data.receiver.socketId).emit("receive-message", {
...
export const sendAttachmentMessage = async (req, res, next) => {
  ...
  try {
    ...
    if (data.newMessage.receiver.socketId) {
      ...
      io.to(data.newMessage.receiver.socketId).emit("receive-message", {
        message: modifiedMessage,
      });
    }
...
export const deleteForEveryone = async (req, res, next) => {
  ...
  try { //Here add contentType and content to unlink files
    const deletedForEveryone = await Message.findOneAndUpdate(
      { _id: message_id },
